  ...
  return (
    ...
    >
      ...
      <h1 className="heading-xl" style={{ fontSize: 'clamp(40px, 7vw, 80px)', color: '#E8EDF2', maxWidth: 820, marginBottom: 20, position: 'relative', zIndex: 1 }}>
        Think smarter.{' '}<br /><span className="text-gradient-green">Work unlimited.</span>
      </h1>
      <p style={{ fontSize: 'clamp(16px, 2vw, 20px)', color: 'rgba(232,237,242,0.6)', maxWidth: 560, marginBottom: 40, lineHeight: 1.65, position: 'relative', zIndex: 1 }}>
        Infinity AI lives inside your browser, works&�offline, keeps your data in South Africa, and costs nothing.
      </p>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 20px', borderRadius: 12, border: '1px solid rgba(232,237,242,0.1)', background: 'rgba(21,32,48,0.8)', backdropFilter: 'blur(12px)', marginBottom: 44, maxWidth: 560, width: '100%', position: 'relative', zIndex: 1 }}>
        ...
        <span style={{ fontSize: 14, color: 'rgba(232,237,242,0.45)', flexShrink: 0 }}>∞ Ask anything:</span>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center', position: 'relative', zIndex: 1 }}>
        <Link href="https://github.com/Sakhileziya/vela-extension" target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 32px', borderRadius: 12, background: 'linear-gradient(135deg, #1B7A4A 0%, #22974F 100%)', color: 'white', fontWeight: 700, fontSize: 15, textDecoration: 'none', boxShadow: '0 0 32px rgba(27,122,74,0.35)', transition: 'transform 0.15s, box-shadow 0.15s' }}>↓ Download Extension — Free</Link>
        <Link href="#features" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 28px', borderRadius: 12, border: '1px solid rgba(232,237,242,0.12)', background: 'rgba(21,32,48,0.6)', color: 'rgba(232,237,242,0.8)', fontWeight: 600, fontSize: 15, textDecoration: 'none', transition: 'background 0.2s, border-color 0.2s' }}>Explore Features →</Link>
      </div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, justifyContent: 'center', marginTop: 44, position: 'relative', zIndex: 1 }}>
        {['POPIA Compliant', 'Zero API Cost', 'Runs Offline', 'Open Source', '100% Local Data'].map((badge) => (
          <span key={badge} style={{ fontSize: 12, fontWeight: 600, color: 'rgba(232,237,242,0.45)', display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ color: '#22974F' }}>✓</span>{badge}</span>
        ))}
      </div>
    </section>
  )
